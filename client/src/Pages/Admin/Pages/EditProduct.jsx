...
const EditProduct = ({ setActiveComponent, productId }) => {
    ...
    useEffect(() => {
        ...
        const fetchProductData = async () => {
            try {
                ...
                const imageFileList = product.images
    ...
    const onFinish = async (values) => {
        try {
            ...
            // Lọc ra ảnh mới để upload
            const newImages = values.image.filter((file) => file.originFileObj);
            if (newImages.length > 0) {
                const uploadedUrls = await handleUpload(newImages);
                imageUrls = [...imageUrls, ...uploadedUrls]; // Chỉ thêm ảnh mới
            }

            // Giữ lại ảnh cũ nhưng tránh bị trùng lặp
            const oldImages = values.image.filter((file) => file.url).map((file) => file.url);

            imageUrls = [...new Set([...oldImages, ...imageUrls])]; // Xóa trùng lặp
